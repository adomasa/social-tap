...
public class PixelCounter
{
   ...
   */
    ...
    public int GetPercentageOfTargetPixels()
    {
        ...
        int height = myImage.Height; // aukštis
        int width = myImage.Width; // plotis
        ...
        checkPixel = myBitmap.GetPixel((int)pixelX, (int)pixelY);
    ...
    public void DrawLineUp(int? levelofBearUp, int xLeft, int width)
    {
        for (int i = ((int)levelofBearUp - 1); i < ((int)levelofBearUp + 2); i++)
        {
            ...
            {
                myBitmap.SetPixel(j, i, Color.Red);
            }
        }
    ...
    public void FindBeerLevel(int height, int width, int xLeft, int yDown, int yUp, ref int? levelofBearDown, ref int? levelofBearUp, ref int[] proc, ref int times)
    {
        ...
        for (int i = (xLeft + distanceX); i < (width + xLeft); i = (i + distanceX)) // plotis
        {
            for (int j = yDown; j > yUp; j--) // aukštis
            {
                ...
                if (myBitmap.GetPixel(i, j).R > lagerBeerRmin && myBitmap.GetPixel(i, j).G > lagerBeerGmin && myBitmap.GetPixel(i, j).G < lagerBeerGmax && myBitmap.GetPixel(i, j).B < lagerBeerBmax && !startBear)
                {
                    startBear = true; // reiškias baigėsi dugnas ir prasideda alus
                    levelofBearDown = j;
                    levelofBearUp = j;
                }
                if (myBitmap.GetPixel(i, j).R > lagerBeerRmin && myBitmap.GetPixel(i, j).G > lagerBeerGmin && myBitmap.GetPixel(i, j).G < lagerBeerGmax && myBitmap.GetPixel(i, j).B < lagerBeerBmax)
                {
                    levelofBearUp = j;
                }
                else
                {
                    ...
                    k = 0;
                    j1 = j;
                    while (k < distanceY && !(myBitmap.GetPixel(i, j1).R == outsideBox.R && myBitmap.GetPixel(i, j1).G == outsideBox.G && myBitmap.GetPixel(i, j1).B == outsideBox.B)) // ar gerai?
                    {
                        if (myBitmap.GetPixel(i, j1).R > lagerBeerRmin && myBitmap.GetPixel(i, j1).G > lagerBeerGmin && myBitmap.GetPixel(i, j1).G < lagerBeerGmax && myBitmap.GetPixel(i, j1).B < lagerBeerBmax)
                        {
                            count++;
                        }
                        j1--;
